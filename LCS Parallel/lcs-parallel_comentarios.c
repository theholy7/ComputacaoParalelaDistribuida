...
int main(int argc, char *argv[])
{
    ...
    else
    {
		...
		/*chi: aqui nao é preciso por + 1 char para o '\0' ou os size_of_vector[0] e size_of_vector[1] já incluem isso?*/
	...
	/*chi: estas a fazer uma matriz de inteiros, para que e que precisas do '\0'? tambem o vais comparar?
			não devia ser: unsigned short int **Matrix = (unsigned short int **) malloc( (size_of_vector[0] + 1) * sizeof(unsigned short int *));
			(devia :p )*/
	

    /* Loop to populate the matrix and give us the longest common subsequence size */
    for (i = 0; i < (size_of_vector[0] + 1); i++) /*chi: se acabares so por alocar size_of_vector[0], tirar o +1 do for*/ 
    {

        Matrix[i] = (unsigned short int *) malloc( (size_of_vector[1] + 1) * sizeof(unsigned short int));
		/*chi: a mesma coisa com o +1*/

        for (j = 0; j < (size_of_vector[1] + 1); j++)
        {
            if (i == 0 || j == 0)
            {
                Matrix[i][j] = 0;	/*chi: porque??? deve haver uma maneira melhor de fazer isto...
											repara que vais ficar com uma matriz que vai ter a 1ª linha e a 1ª coluna so com zeros...
											gasto de memoria parvo? não percebo nada de cpd, atençao!*/
            }
	...
	/*chi: daqui para baixo já não vi com atenção...*/
